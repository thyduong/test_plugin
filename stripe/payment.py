...
class StripeSettingsHolder(BasePaymentProvider):
    ...
    def get_connect_url(self, request):
        ...
        return (
            ...
            urllib.parse.quote(build_global_uri('plugins:stripe2:oauth.return')),
        )
        ...
        if self.settings.connect_client_id and not self.settings.secret_key:
            ...
            if not self.settings.connect_user_id:
                ...
                return (
                    ...
                    reverse('plugins:stripe2:oauth.disconnect', kwargs={
            ...
            return "<div class='alert alert-info'>%s<br /><code>%s</code></div>" % (
                ...
                build_global_uri('plugins:stripe2:webhook')
            )
...
class StripeMethod(BasePaymentProvider):
    ...
    def payment_pending_render(self, request, payment) -> str:
        ...
        ctx = {
            ...
            'payment_hash': payment.order.tagged_secret('plugins:stripe2')
        }
        return template.render(ctx)
    ...
    def redirect(self, request, url):
        ...
        if request.session.get('iframe_session', False):
            ...
            redirect_url = build_absolute_uri(request.event, 'plugins:stripe2:redirect')
    ...
    def _handle_payment_intent(self, request, payment, intent=None):
        ...
        def create_payment_intent(payment_method_id, idempotency_key_seed):
            ...
            return stripe.PaymentIntent.create(
                ...
                return_url=build_absolute_uri(self.event, 'plugins:stripe2:sca.return', kwargs={
                    'order': payment.order.code,
                    'payment': payment.pk,
                    'hash': payment.order.tagged_secret('plugins:stripe2')
                }),
    ...
    def _redirect_to_sca(self, request, payment):
        url = build_absolute_uri(self.event, 'plugins:stripe2:sca', kwargs={
            'order': payment.order.code,
            'payment': payment.pk,
            'hash': payment.order.tagged_secret('plugins:stripe2'),
        })
        ...
        if not self.redirect_in_widget_allowed and request.session.get('iframe_session', False):
            redirect_url = build_absolute_uri(self.event, 'plugins:stripe2:redirect')
...
class StripeMultibanco(StripeSourceMethod):
    ...
    def _create_source(self, request, payment):
        source = stripe.Source.create(
            ...
            redirect={
                'return_url': build_absolute_uri(self.event, 'plugins:stripe2:return', kwargs={
                    'order': payment.order.code,
                    'payment': payment.pk,
                    'hash': payment.order.tagged_secret('plugins:stripe2'),
                })
