...
from .payment import StripeCC, StripeSettingsHolder
...
def redirect_view(request, *args, **kwargs):
    ...
    if 'go' in request.GET:
        ...
        r = render(request, 'plugins/stripe/redirect.html', {
            'url': build_absolute_uri(request.event, 'plugins:stripe2:redirect') + '?' + urllib.parse.urlencode(params),
        })
        r._csp_ignore = True
        return r
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        try:
            self.order = request.event.orders.get_with_secret_check(
                code=kwargs['order'], received_secret=kwargs['hash'].lower(), tag='plugins:stripe2'
            )
        except Order.DoesNotExist:
...
class OrganizerSettingsFormView(DecoupleMixin, OrganizerDetailViewMixin, AdministratorPermissionRequiredMixin, FormView):
    ...
    def get_success_url(self):
        return reverse('plugins:stripe2:settings.connect', kwargs={
